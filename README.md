...
### 기능
- **난이도 50단계** — 레벨이 오를수록 격자가 커지고(8×8 → 16×16), 대각선·역방향이 추가되고, 단어가 길고 많아집니다(레벨당 6→14개 출제).
- **매 게임 다른 단어** — 각 레벨마다 같은 주제의 큰 단어 풀(레벨당 15~28개)에서 매 게임 무작위로 한 묶음을 뽑아 배치하므로, 같은 레벨을 다시 해도 단어와 위치가 매번 바뀝니다.
